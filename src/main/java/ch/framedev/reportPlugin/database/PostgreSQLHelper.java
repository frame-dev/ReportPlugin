...
public class PostgreSQLHelper implements DatabaseHelper {
    ...
    /**
     * Initializes the postgreSQLHelper with the specified ReportPlugin instance.
     * Connects to the postgreSQL database using configuration from the plugin.
     *
     * @param plugin The ReportPlugin instance for logging and configuration access.
     */
    public PostgreSQLHelper(ReportPlugin plugin) {
        String host = plugin.getConfig().getString("postgresql.host", "localhost");
        String database = plugin.getConfig().getString("postgresql.database", "database");
        String username = plugin.getConfig().getString("postgresql.username", "root");
        String password = plugin.getConfig().getString("postgresql.password", "password");
        int port = plugin.getConfig().getInt("postgresql.port", 3306);
        this.postgreSQL = new PostgreSQL(host, port, database, username, password);
        if (!connect()) {
            plugin.getLogger().severe("Could not connect to the PostgreSQL database. Disabling plugin.");
            plugin.getServer().getPluginManager().disablePlugin(plugin);
            return;
        }
        plugin.getLogger().info("Connected to PostgreSQL database successfully.");
        plugin.getLogger().info("Creating reports table...");
        createTable();
        plugin.getLogger().info("PostgreSQL database initialized successfully.");
    }

    /**
     * Creates the reports table in the postgreSQL database if it does not already exist.
    ...
    public boolean connect() {
        try(Connection connection = postgreSQL.connect()) {
            ...
            ReportPlugin.getInstance().getLogger().log(Level.SEVERE, "Error connecting to postgreSQL database", ex);
            return false;
        }
    }
